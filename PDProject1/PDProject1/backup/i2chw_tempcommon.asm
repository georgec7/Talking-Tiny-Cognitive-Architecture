...
export    I2CHW_Temp_InitWrite
export   _I2CHW_Temp_InitWrite
export    I2CHW_Temp_InitRamRead
export   _I2CHW_Temp_InitRamRead
export    I2CHW_Temp_InitFlashRead
export   _I2CHW_Temp_InitFlashRead
export    I2CHW_Temp_bReadI2CStatus
export   _I2CHW_Temp_bReadI2CStatus
export    I2CHW_Temp_ClrRdStatus
export   _I2CHW_Temp_ClrRdStatus
export    I2CHW_Temp_ClrWrStatus
export   _I2CHW_Temp_ClrWrStatus
...
.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: I2CHW_Temp_InitWrite
;
;  DESCRIPTION:
...
;
 ...
 I2CHW_Temp_InitWrite:
_I2CHW_Temp_InitWrite:
...
WrBufLo: equ -3
    ...
    and    [I2CHW_Temp_bStatus], ~fI2C_NAKnextWr                     ;reset the nak-next-written-byte flag.
IF SYSTEM_LARGE_MEMORY_MODEL
    mov   A, [X + WrBufHi]                                           ;move wrbuf addr to A
    mov   [pI2CHW_Temp_Write_BufHI], A
ENDIF
    mov   A, [X + WrBufLo]                                           ;move wrbuf addr to A
    mov   [pI2CHW_Temp_Write_BufLO], A
    mov   A, [X + WrCnt]                                               ;move Write_count to A
    mov   [I2CHW_Temp_Write_Count], A
    cmp   A, 01                                                        ;if data buffer is one byte long or less
    jz    I2CHW_Temp_SetNak
    jc    I2CHW_Temp_SetNak
    and   [I2CHW_Temp_RsrcStatus], ~0x70                             ;clear the 0x10, 0x20 (Write status bits)
    jmp   I2Cs_1_ResetFlgs

I2CHW_Temp_SetNak:
    or    [I2CHW_Temp_bStatus], fI2C_NAKnextWr                       ;set the nak-next-written-byte flag.
    and   [I2CHW_Temp_RsrcStatus], ~0x70                             ; clear the 0x10, 0x20 (Write status bits)
    jmp   I2Cs_1_ResetFlgs

I2Cs_1_ResetFlgs:
    pop A
    and A, I2CHW_Temp_INT_MASK                                                ; Only enable if it was previously enabled
    jz  . + 5
...
.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: I2CHW_Temp_InitRamRead
;
;  DESCRIPTION:
...
;
 ...
 I2CHW_Temp_InitRamRead:
_I2CHW_Temp_InitRamRead:
...
RdBufLo:   equ   -3
    ...
    RAM_PROLOGUE RAM_USE_CLASS_2
	RAM_PROLOGUE RAM_USE_CLASS_4
	RAM_SETPAGE_CUR >I2CHW_Temp_Read_Count
    push  X
	mov   X, SP
    dec   X                                                          ;set up the pointer for correct param access
    push  A
...
IF SYSTEM_LARGE_MEMORY_MODEL
    ...
    mov   [pI2CHW_Temp_Read_BufHI], A
ENDIF
    mov   A, [X + RdBufLo]                                             ; move rdbuf addr to A
    mov   [pI2CHW_Temp_Read_BufLO], A
    mov   A, [X + RdCnt]                                               ; move RamRead_count to A
    mov   [I2CHW_Temp_Read_Count], A
    dec   [I2CHW_Temp_Read_Count]                                    ; since we decrement through zero...
    and   [I2CHW_Temp_RsrcStatus], ~0x0f                             ; clear the lower 4 (read status bits)
...
.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: I2CHW_Temp_InitFlashRead
;
;  DESCRIPTION:
...
;     either case
;
 I2CHW_Temp_InitFlashRead:
_I2CHW_Temp_InitFlashRead:
...
IF I2CHW_Temp_READ_FLASH
    RAM_PROLOGUE RAM_USE_CLASS_4
    RAM_PROLOGUE RAM_USE_CLASS_2
    RAM_SETPAGE_CUR >pI2CHW_Temp_Read_BufLO   
	push  X
	mov   X, SP
    dec   X                                                            ;set up the pointer for correct param access
    push  A
    ...
    mov   [I2CHW_Temp_Read_CountHI], A

    dec   [I2CHW_Temp_Read_Count]                                    ; since this will count throu zero
    jnc    . + 4                                                       ; only Read_CountHI if Read_Count rolled to 0xff
    dec   [I2CHW_Temp_Read_CountHI]

    and   [I2CHW_Temp_RsrcStatus], ~0x07                             ; clear the lower 3 (read status bits)
    or    [I2CHW_Temp_RsrcStatus],I2CHW_READFLASH                    ; set the flash status bit
...
.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: I2CHW_Temp_bReadI2CStatus
;
;  DESCRIPTION:
...
;          CUR_PP
;          
 I2CHW_Temp_bReadI2CStatus:
_I2CHW_Temp_bReadI2CStatus:
    RAM_PROLOGUE RAM_USE_CLASS_4
	RAM_SETPAGE_CUR >I2CHW_Temp_RsrcStatus
    mov   A, [I2CHW_Temp_RsrcStatus]                                 ;return the status in A
    RAM_EPILOGUE RAM_USE_CLASS_4
    ret
...
.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: I2CHW_Temp_ClrRdStatus
;
;  DESCRIPTION:
...
;          CUR_PP
;          
 I2CHW_Temp_ClrRdStatus:
_I2CHW_Temp_ClrRdStatus:
    RAM_PROLOGUE RAM_USE_CLASS_4
	RAM_SETPAGE_CUR >I2CHW_Temp_RsrcStatus
    and   [I2CHW_Temp_RsrcStatus], ~0x07                             ; clear the lower 3 (read status bits)
    RAM_EPILOGUE RAM_USE_CLASS_4
...
.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: I2CHW_Temp_ClrWrStatus
;
;  DESCRIPTION:
;     Clears the Status bits in the Control/Status register, doesn't alter buffer
;     addresses or counts, leaves the Flash/Ram Read bit alone too
...
;          CUR_PP
;          
 I2CHW_Temp_ClrWrStatus:
_I2CHW_Temp_ClrWrStatus:
    RAM_PROLOGUE RAM_USE_CLASS_4
	RAM_SETPAGE_CUR >I2CHW_Temp_RsrcStatus
    and   [I2CHW_Temp_RsrcStatus], ~0x70                             ; clear bits 10,20 (write status bits)
    RAM_EPILOGUE RAM_USE_CLASS_4
    ret
